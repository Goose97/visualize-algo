import { Instructions, initQueue } from './helper';
import { QueueOperation, EnqueueParams } from './index.d';
import { StepInstruction } from 'types';

export const queueInstruction = (
  data: number[],
  ...
  switch (operation) {
    case 'enqueue':
      return enqueueInstruction(data, parameters);

    case 'dequeue':
      return dequeueInstruction(data);
...
const enqueueInstruction = (data: number[], { value }: EnqueueParams) => {
  const linkedList = initQueue(data);
  // const _getExplanationAndCodeLine = getExplanationAndCodeLine.bind(
  //   null,
  //   'search',
  // );
  let instructions = new Instructions();
  instructions.push({ actions: [{ name: 'enqueue', params: [value] }] });

  return instructions.get();
};

const dequeueInstruction = (data: number[]) => {
  const linkedList = initQueue(data);
  // const _getExplanationAndCodeLine = getExplanationAndCodeLine.bind(
  //   null,
  //   'search',
  // );
  let instructions = new Instructions();
  instructions.push({ actions: [{ name: 'dequeue', params: [] }] });
